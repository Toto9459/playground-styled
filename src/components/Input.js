    ...
    useEffect(() => {
      if (textSave.length > 0) {
        ...
        if (goodGroup.length === 3 && verificationLenght === true) {
          setErrorMsg("C'est du solide !");
          setStatusColor('Vert')
          return;
        }
        // Medium >= 8 caractères & 2 groupe //
        if (goodGroup.length === 2 && verificationLenght === true) {
          setErrorMsg("Pour obtenir un mot de passe plus solide, ajoutez quelques caractères, chiffres ou symboles. ");
          setStatusColor('Orange')
          return;
        }
        // Weak >= 8 caractères & 1 groupe //
        if (goodGroup.length === 1 && verificationLenght === true) {
          setErrorMsg("Votre mot de passe doit contenir au moins 2 des 4 éléments suivant ; majuscule, minuscule, chiffre, symbole.");
  ...
  return (
    <Parent>
      ...
      <Title>Client Password</Title>
        ...
        <PasswordInput type="password" id="password" value={textSave}  placeholder="8 caractères minimum" required onKeyDown={noSpace} onChange={entry} />
